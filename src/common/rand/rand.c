...
#endif
void OQS_randombytes_qrng(uint8_t *random_array, size_t bytes_to_read);
...
OQS_API OQS_STATUS OQS_randombytes_switch_algorithm(const char *algorithm) {
        if (0 == strcasecmp(OQS_RAND_alg_system, algorithm)) {
                ...
                return OQS_ERROR;
#endif
        } else if (0 == strcasecmp(OQS_RAND_alg_qrng, algorithm)) {
                oqs_randombytes_algorithm = &OQS_randombytes_qrng;
                return OQS_SUCCESS;
        } else {
                return OQS_ERROR;
        }
...
void OQS_randombytes_qrng(uint8_t *random_array, size_t bytes_to_read) {
	static const uint8_t pattern[4] = {0xCA, 0xFE, 0xBA, 0xBE};
	for (size_t i = 0; i < bytes_to_read; i++) {
		random_array[i] = pattern[i % 4];
	}
	}
